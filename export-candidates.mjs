...
function termPrefix(c) {
  return NAMESPACE_PREFIX[c.namespace] ?? (c.ontology === 'TO' ? 'TO' : 'PO');
}

// Placeholder ID — curators replace with real accession
function placeholderId(c, index) {
  return `${termPrefix(c)}:NEWTERM_${String(index + 1).padStart(3, '0')}`;
}
...
function buildGithubIssue(c, index) {
  const onto  = c.ontology === 'TO' ? 'TO' : 'PO';
  const repo  = GITHUB_REPOS[onto] ?? GITHUB_REPOS.PO;
  const lines = [
    `## New Term Request — ${onto}`,
    '',
    `**Proposed label:** ${c.term}`,
    `**Ontology:** ${onto === 'PO' ? 'Plant Ontology (PO)' : 'Trait Ontology (TO)'}`,
...
if (formats.includes('github')) {
  const ontos = [...new Set(candidates.map(c => c.ontology === 'TO' ? 'TO' : 'PO'))];
  console.log('\nSubmission links:');
